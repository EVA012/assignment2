...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dxhat * (x - mean) * (a**-3) * -0.5 , axis=0)
...
def layernorm_backward(dout, cache):
    ...
    a = np.sqrt(var + eps)
    dxhat = dout * gamma
    dvar = np.sum((x - mean) * dxhat * -0.5 / a**3, axis=1)
    dmean = np.sum( - dxhat / a, axis=1) + dvar * np.sum(-2 * (x - mean),axis=1)/N
    dx = dxhat / a  + dvar.reshape(-1,1) * 2 * (x - mean) / N + dmean.reshape(-1,1) / N
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    #pass
    N, C, H, W = x.shape
    tem, cache = batchnorm_forward(x.transpose(0,2,3,1).reshape((N*H*W,C)), gamma, beta, bn_param)
    out = tem.reshape(N, H, W, C).transpose(0,3,1,2)
